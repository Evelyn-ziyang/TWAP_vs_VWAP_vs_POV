...
本报告的结论不是某一种算法全面胜出，而是三种方法承担不同类型的执行风险。\algoTWAP{} 的路径最平滑、交易前可预测性最高，但忽略日内流动性曲线；\algoVWAP{} 利用交易前估计的 volume profile 调整节奏，在当前短窗口样本中与 TWAP 的总体表现非常接近，其主要风险来自成交量曲线预测误差；\algoPOV{} 对实际放量和缩量反应最快，完成时点最灵活，但成本分布更宽，对成交量和波动率 regime 更敏感。

\keybox{\textbf{主要发现：}在 115 个 Dynamic-Q 窗口中，Forecast VWAP 相对 TWAP 的 mean $\Delta IS$ 为 $+0.02$ bps、胜率为 57.4\%；POV 的 mean $\Delta IS$ 为 $+0.84$ bps、胜率为 51.3\%。三种算法 completion 均为 100\%，因此相对成本差异不是由 unequal fill ratio 驱动。}

单日固定母订单案例中，当日价格自 arrival 后明显下跌。较晚成交的 TWAP 因此获得更低的 BUY 成交价，而较早完成的 POV 最接近全天 market VWAP。这一结果主要反映成交时点与单日价格路径的交互，不应解释为 TWAP 稳定产生 alpha。算法评价必须先明确 benchmark：贴近 market VWAP、降低 arrival shortfall、控制 modeled impact 和确保按时完成，是不同的目标。
...
POV 随已观察市场成交量更新子单：放量时加速，缩量时减速。因此它不是交易前一次性固定的 schedule，完成时点也不是常数。若窗口市场量不足，它可以留下未完成量；在事件放量时，它又可能快速集中成交并增加价格路径敏感性。
...
POV & Realized volume & 不能 & 适应实际放量和缩量 & 完成时间与路径风险 \\
\bottomrule
\end{tabularx}
\end{table}
...
Dynamic-Q 使用 115 个相互重叠的 60-minute 窗口，每个窗口包含 12 根 5-minute bar，每 15 分钟滚动一次。每个窗口的母订单设置为：

\begin{equation}
Q_w=\left\lfloor 3\%\times\sum_i V_{w,i}\right\rfloor.
\end{equation}

样本中 $Q_w$ 范围为 170 至 1,217，中位数为 397。该设计让不同窗口的订单量与窗口成交量同比例变化，从而近似控制订单难度。不过，它使用完整窗口的 realized volume，是 ex-post research normalization，不是实盘可提前获得的 sizing 规则。
...
数值越低越好。算法相对 TWAP 的差值为 $\Delta IS=IS_{\mathrm{algo}}-IS_{\mathrm{TWAP}}$，因此 $\Delta IS<0$ 表示被比较算法优于 TWAP。所有算法都接受统一的 20\% bar-volume hard cap；计划量未成交时会 carry forward。
...
\midrule
TWAP & -2.29 bps & 0.00 bps & -- & 100.0\% \\
Forecast VWAP & -2.26 bps & +0.02 bps & 57.4\% & 100.0\% \\
POV & -1.45 bps & +0.84 bps & 51.3\% & 100.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
图 \ref{fig:fullday} 给出三种算法最直观的区别。\algoTWAP{} 每 bar 目标接近常数，累计成交量接近直线；\algoVWAP{} 根据历史 profile 在预测高量时段多做、低量时段少做，累计路径相对 TWAP 发生弯曲；\algoPOV{} 紧跟当日 observed volume，在开盘放量阶段快速成交并明显提前完成，完成后 schedule 归零。
...
当天价格从 arrival 后明显下跌。对 BUY 订单而言，较晚执行自然更容易获得低价，因此 TWAP 的成交均价低于较早完成的 POV。另一方面，POV 的成交路径最贴近当日成交量，因而最接近 market VWAP；Forecast VWAP 的模型化 spread、impact 与 fee 合计最低。这里出现三个不同的评价角度：成交价格、VWAP tracking 和模型成本不会自动给出同一排名。
...
在图 \ref{fig:single} 中，三种算法均完成订单，但 TWAP 将交易均匀延伸到窗口末，Forecast VWAP 只做温和的 profile 倾斜，POV 则因窗口开头放量而在前几个 bar 快速完成。这表明 POV 的核心不是一种静态权重，而是 realized-volume feedback 和可变完成时点。
...
图 \ref{fig:delta} 显示 Forecast VWAP 的差值紧密集中在 0 附近，意味着这些短窗口中它与 TWAP 的执行时点通常接近。Forecast VWAP 虽在 57.4\% 的窗口获胜，但少量较大的正向损失使 mean $\Delta IS$ 为 $+0.02$ bps；其 median $\Delta IS$ 为 $-0.08$ bps。胜率和均值方向不一致，说明尾部窗口影响了平均值。

POV 的分布更宽、尾部更明显。其 median $\Delta IS$ 为 $-0.26$ bps，但 mean $\Delta IS$ 为 $+0.84$ bps，胜率为 51.3\%。这种差异来自 POV 对窗口内成交量顺序和价格路径的即时反应：同样的窗口总量，如果放量发生在不同价格阶段，成交时点就会明显变化。

\subsection{为什么分布集中仍然有研究意义}

Forecast VWAP 与 TWAP 的 $\Delta IS$ 密集在 0 附近并不是绘图错误，而是实验结构的结果：窗口只有 60 分钟、订单只占窗口 volume 的 3\%、三种算法都在 20\% hard cap 下完整成交；同时，短窗口内历史 profile 的倾斜程度有限。因此，二者 schedule 的差异小于共同的市场价格移动。此时更应观察尾部、median 和 regime，而不应只放大横轴来制造视觉差异。
...
Forecast VWAP 在 low-volume/low-vol 和 high-volume/low-vol regime 中平均略优于 TWAP，mean $\Delta IS$ 分别为 $-0.25$ 和 $-0.21$ bps；在 low-volume/high-vol regime 中 mean $\Delta IS$ 为 $+0.47$ bps，胜率仅 25.0\%。这说明历史 profile 可以改变计划节奏，但无法保证在高波动价格路径下取得优势。

\subsection{POV：Regime 敏感性明显更强}

POV 在 low-volume/low-vol regime 表现最好，mean $\Delta IS$ 为 $-2.56$ bps、胜率为 64.9\%；但在 high-volume/low-vol 和 high-volume/high-vol regime 中 mean $\Delta IS$ 分别为 $+3.91$ 和 $+2.68$ bps。一个可能的机制是：POV 追随放量，会把成交集中到放量出现的价格阶段，从而增强 timing path dependence。这里是由图表和算法结构得到的解释性推断，并非因果识别。

图中所有 regime 的 completion 都是 100\%。因此，左上角 Forecast VWAP 的结果接近 0，含义是它与 TWAP 在该 regime 的平均成交时点和成本非常接近，而不是某种算法没有完成订单。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{04_is_distribution_by_algorithm.png}
\caption{三种算法在 115 个 Dynamic-Q 窗口中的 implementation-shortfall 分布。Whisker 使用 $1.5\times IQR$，隐藏 outlier。}
\label{fig:box}
\end{figure}

TWAP 与 Forecast VWAP 的箱体高度重叠，再次表明 Forecast VWAP 在当前样本中没有稳定的 unconditional 优势。POV 的中心和离散程度有所不同，来自其更快、更加依赖 realized volume 的完成路径。三组 mean IS 分别为 $-2.29$、$-2.26$ 和 $-1.45$ bps，但这些均值同时包含窗口价格移动和模型化交易成本，不能被单独解释为算法 alpha。

\begin{figure}[H]
\centering
\includegraphics[width=0.60\textwidth]{05_completion_rate.png}
\caption{Dynamic-Q 的平均 completion rate。}
\label{fig:completion}
\end{figure}

三种算法在全部 115 个窗口中均实现 100\% completion，因此图 \ref{fig:delta} 至图 \ref{fig:box} 的成本比较不是用较低 fill ratio 换取较好 IS。该结论只适用于当前的 3\% order fraction、10\% POV rate 和 20\% hard cap；若提高订单比例或缩短窗口，POV 的未完成风险会重新出现。
...
\textbf{维度} & \textbf{TWAP} & \textbf{Forecast VWAP} & \textbf{POV} \\
\midrule
执行节奏 & 最平滑、最可预测 & 向预测高量时段倾斜 & 对实际放量快速响应 \\
完成时点 & 通常接近窗口末 & 通常接近窗口末 & 可提前，也可能因低量延迟 \\
当前样本 & 稳定 baseline & 与 TWAP 接近，局部小幅改善 & 分布更宽，regime 敏感更强 \\
主要风险 & 流动性错配 & Volume-profile forecast error & Realized-volume 路径与完成风险 \\
适用目标 & 按时间均匀执行 & 跟随预期市场节奏 & 维持实时市场参与率 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
  \item \textbf{先说明 benchmark。} 如果客户目标是接近 market VWAP，单日中 POV 的结果更接近该 benchmark；如果目标是降低 arrival IS，结果会强烈依赖价格路径。
  \item \textbf{同时汇报 full-day 与 Dynamic-Q。} 前者回答“一笔固定订单如何执行”，后者回答“控制订单难度后的成本分布如何”，二者不能互相替代。
  \item \textbf{同时汇报 mean、median、win rate 和 completion。} Forecast VWAP 的正胜率与略正 mean $\Delta IS$ 正是尾部影响的例子。
...
\begin{enumerate}
  \item \textbf{样本不足。} 当前只有 AAPL 和 6 个交易日；115 个窗口存在 75\% overlap，不能视为独立样本做显著性推断。
  \item \textbf{Volume 口径有限。} Tick volume 不是 consolidated shares，POV rate、Dynamic-Q 与 impact 只适合方法展示。
  \item \textbf{Forecast VWAP 较简单。} Profile 只使用有限历史日的中位数，没有 weekday、event、auction、regime conditioning 或实时 reforecast。
  \item \textbf{成交模型简化。} 当前使用 bar typical price、固定 half-spread、平方根 impact 和 fee，没有 order book、queue、venue、partial fill 与 adverse selection。
